...
filealloc(void)
{
  ...
  for(f = ftable.file; f < ftable.file + NFILE; f++){ // 顺序扫描所有文件槽位
    if(f->ref == 0){ // 找到引用计数为0的空闲槽位
...
fileclose(struct file *f)
{
  ...
  if(ff.type == FD_PIPE){ // 管道类型：由 pipe 子系统完成清理
    pipeclose(ff.pipe, ff.writable); // 根据读/写方向做关闭
  } else if(ff.type == FD_INODE || ff.type == FD_DEVICE){ // inode 或设备需要文件系统事务
...
filestat(struct file *f, uint64 addr)
{
  ...
  if(f->type == FD_INODE || f->type == FD_DEVICE){ // 只有 inode/设备才有 stat 信息
