...
sys_read(void)
{
  ...
  //
  // 在这里，argaddr(1, &p)的具体工作原理：
  // 1. 参数1对应RISC-V架构中的a1寄存器
  // 2. a1寄存器存储的是read系统调用的第二个参数：缓冲区地址
  // 3. argaddr函数内部调用argraw(1)从陷阱帧中获取a1寄存器的值
  // 4. 将这个值（用户空间的缓冲区地址）存入p变量
  //
  // 例如，当用户程序调用read(fd, buf, size)时：
  // - a7寄存器存储SYS_read系统调用号
  // - a0寄存器存储文件描述符fd（第一个参数）
  // - a1寄存器存储buf缓冲区的地址（第二个参数）
  // - a2寄存器存储要读取的字节数size（第三个参数）
...
sys_write(void)
{
  ...
  // 在这里，argaddr(1, &p)的具体工作原理：
  // 1. 参数1对应RISC-V架构中的a1寄存器
  // 2. a1寄存器存储的是write系统调用的第二个参数：缓冲区地址
  // 3. argaddr函数内部调用argraw(1)从陷阱帧中获取a1寄存器的值
  // 4. 将这个值（用户空间的缓冲区地址）存入p变量
  //
  // 例如，当用户程序调用write(fd, buf, size)时：
  // - a7寄存器存储SYS_write系统调用号
  // - a0寄存器存储文件描述符fd（第一个参数）
  // - a1寄存器存储buf缓冲区的地址（第二个参数）
  // - a2寄存器存储要写入的字节数size（第三个参数）
  // - 这里argaddr(1, &p)就是获取a1寄存器的值，即buf缓冲区的地址
  //
  // 这正是kernel/syscall.c中case 1: return p->trapframe->a1;的具体应用场景
...
sys_fstat(void)
{
  ...
  //
  // 在这里，argaddr(1, &st)的具体工作原理：
  // 1. 参数1对应RISC-V架构中的a1寄存器
  // 2. a1寄存器存储的是fstat系统调用的第二个参数：stat结构体的地址
  // 3. argaddr函数内部调用argraw(1)从陷阱帧中获取a1寄存器的值
  // 4. 将这个值（用户空间的stat结构体地址）存入st变量
  //
  // 例如，当用户程序调用fstat(fd, &st)时：
  // - a7寄存器存储SYS_fstat系统调用号
  // - a0寄存器存储文件描述符fd（第一个参数）
  // - a1寄存器存储&st结构体的地址（第二个参数）
  // - 这里argaddr(1, &st)就是获取a1寄存器的值，即stat结构体的地址
  //
  // 这再次展示了kernel/syscall.c中case 1: return p->trapframe->a1;的应用
  argaddr(1, &st);
  if (argfd(0, 0, &f) < 0)
    return -1;
...
sys_exec(void)
{
  ...
  // `argaddr` 会查看当前陷入帧（trap frame）中系统调用的第 1 个参数，
  // 把它当作用户态地址解析并写入 `uargv`，顺便检查该指针是否落在用户地址空间。
  // 最终 `uargv` 成为内核侧访问用户参数向量的入口，用来逐个取回 argv[i]。
  //
  // 在这里，argaddr(1, &uargv)的具体工作原理：
  // 1. 参数1对应RISC-V架构中的a1寄存器
  // 2. a1寄存器存储的是exec系统调用的第二个参数：argv数组的地址
  // 3. argaddr函数内部调用argraw(1)从陷阱帧中获取a1寄存器的值
  // 4. 将这个值（用户空间的地址）存入uargv变量
  //
  // 例如，当用户程序调用exec("/bin/echo", argv)时：
  // - a7寄存器存储SYS_exec系统调用号
  // - a0寄存器存储"/bin/echo"字符串的地址（第一个参数）
  // - a1寄存器存储argv数组的地址（第二个参数）
  // - 这里argaddr(1, &uargv)就是获取a1寄存器的值，即argv数组的地址
  //
  // 这正是kernel/syscall.c中case 1: return p->trapframe->a1;的具体应用场景
  ...
  // - path：内核空间的缓冲区，用于存储从用户空间复制过来的路径字符串
